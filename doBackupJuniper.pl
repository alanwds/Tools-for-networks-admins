...
#Script that connect to a juniper router and do a backup. The script also do a rotate of old backup files.
...
sub LOG{
        ...
        open LOG, ">>/var/log/backupJuniper.log" or die $!;
        ...
        my $temp = `/usr/bin/logger -i -p local7.info -t \"Backup Juniper\" $logitem`;
...
sub doBackup{
        ...
        LOG "Realizando Backup do Juniper $ip";
        ...
        $scpe->scp("$ip:/config/juniper.conf.1.gz","/opt/backup/juniper/juniper-$ip-$currentDate\.conf\.gz");
...
sub backupRotate{
        ...
        my $comandoShell = 'find /opt/backup/juniper/ -name "juniper-*" -atime +14 -delete -print';
        ...
        if($stderr){
                LOG "ERRO_BKP_Juniper: Nao foi possivel executar o comando";
                LOG "ERRO_BKP_Juniper: $stderr";
        }else{
